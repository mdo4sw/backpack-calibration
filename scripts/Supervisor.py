...
class main_supervisor:
    def __init__(self):
        ...
        self.startTime = 0.0
        self.endTime = 0.0
        self.realstartTime = 0.0
        self.realendTime = 0.0
        self.startDate = ""
        self.endDate = ""
    ...
    def set_REF_name(self,name):
        self.REF.set_instrumentName(name)
    def get_REF_name(self):
        return self.REF.get_instrumentName()
    ########################################
    ...
    def set_DUT_name(self,name):
        self.DUT.set_instrumentName(name)
    def get_DUT_name(self):
        return self.DUT.get_instrumentName()
   ...
   ################## For Data Analysis ####################
    ...
    def set_analysis_data(self,ref_data,dut_data):
        try:
            ...
            for i in range(0,len(dut_data)):
                ...
                temp_x_data.append(dut_data[temp_seq])
                #print(temp_x_data)
            ...
            for i in range(0,len(ref_data)):
                ...
                temp_y_data.append(ref_data[temp_seq])
                #print(temp_y_data)
    ...
    def plot_figure(self,output_dir):
        x_name = self.DUT.get_instrumentName()
        self.REF.set_instrumentName("1185-FDDS2010")
        y_name = self.REF.get_instrumentName()
        title = self.get_title()
        result = self.get_testResult()
        
        if self.get_DUT_mode() == "idle":
            title = "High Precision"
            self.ANALYSIS.plotting_linear_fit(y_name,x_name,output_dir,title,result)
            self.ANALYSIS.plotting_scatterPlot(y_name,x_name,output_dir,title)
        elif self.get_DUT_mode() == "armed":
            title = "High Range"
            self.ANALYSIS.plotting_linear_fit(y_name, x_name,output_dir,title,result)
            self.ANALYSIS.plotting_scatterPlot(y_name,x_name,output_dir,title)
   ...
   ################### InstrCal File ######################
    ...
    def generate_proposed_InstCalFile(self):
        ...
        #source_file_name = "R:/crd_G2000/_Coordinators/NomardR/DEV/calibration/data/20200122/InstrCal.ini"
        #target_file_name = "R:/crd_G2000/_Coordinators/NomardR/DEV/calibration/data/20200122/InstrCal_proposed.ini"
    ...
    def generate_fit_result(self):
        
            
        self.UTIL.gen_result_ini(self.UTIL.get_result_dir(),
                                 self.DUT.get_instrumentName(),
                                 self.REF.get_instrumentName(),
                                 self.ANALYSIS.get_slope(),
                                 self.ANALYSIS.get_intercept(),
                                 self.get_realstartTime(),
                                 self.get_realendTime(),
                                 self.get_totalTime(),
                                 self.get_startDate(),
                                 self.get_endDate(),
                                 self.get_title(),self.get_testResult())
        
   ########################################################
    def set_title(self): 
        if self.get_DUT_mode() == "idle":
            self.title = "High Precision"
           
        elif self.get_DUT_mode() == "armed":
            self.title = "High Range"
        
    def set_testResult(self):
        slope = self.get_slope()
        if float(slope) > 0.97 or float(slope) < 1.03:
            self.result = "PASS"
        else:
            self.result = "FAIL"
   
    def set_startTime (self):
        self.startTime = self.UTIL.set_timer()
        self.realstartTime = self.UTIL.set_realTime()
        self.startDate = self.UTIL.set_today()
    ...
    def get_realstartTime(self):
        return self.realstartTime
    def get_realendTime(self):
    ...
    def set_endTime(self):
        self.endTime = self.UTIL.end_timer()
        self.realendTime = self.UTIL.set_realTime()
        self.endDate = self.UTIL.set_today()
        
    def get_totalTime(self):
        total_time = self.get_endTime() - self.get_startTime()
        print(total_time)
    ...
    def set_cycle(self):
        ...
        elif self.riseCycle and not self.fallCycle:
            self.riseCycle = False
            self.fallCycle = True
            self.firstSeq = "6.0"
            self.lastSeq = "1.0"
